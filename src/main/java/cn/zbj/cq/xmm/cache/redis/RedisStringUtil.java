package cn.zbj.cq.xmm.cache.redis;
...
public class RedisStringUtil extends RedisBaseUtil{
     ...
     * @return
     */
	public static String getSet(CachePrefix keyPrefix, String key,String value){
		boolean broken = false;//该操作是否被异常打断而没有正常结束
		ShardedJedis jedis = null;
		try {
			jedis = getJedis();//获取jedis实例
			...
			return jedis.getSet(keyPrefix+KEY_SPLIT+key,value);
		} catch (Exception e) {
			broken = true;
		}finally{
			returnJedis(jedis, broken);
